...
const { NodeVM } = require('@zhennann/vm2');
...
const tools = {
  ...
  _getCabloyPath(projectPath) {
    let cabloyPath = path.join(projectPath, 'packages/cabloy');
    if (fs.existsSync(cabloyPath)) return cabloyPath;
    cabloyPath = path.join(projectPath, 'node_modules/cabloy');
    if (fs.existsSync(cabloyPath)) return cabloyPath;
    return null;
  },
  _getVonaPath(projectPath) {
    let vonaPath = path.join(projectPath, 'packages-vona/vona');
    if (fs.existsSync(vonaPath)) return vonaPath;
